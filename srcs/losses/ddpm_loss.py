...
class GaussianDiffusion1D(nn.Module):
    def __init__(
        ...
        *,
        ...
        if beta_schedule == 'linear':
            ...
            betas = cosine_beta_schedule(timesteps) # betas.shape - (timesteps, 1)
        else:
            raise ValueError(f'unknown beta schedule {beta_schedule}')

        self.beta = torch.linspace(0.0001, 0.02, timesteps) ## Made adjustment 
    ...
    def p_losses(self, x_start, t, cond=None, noise = None):
        ...
        with torch.no_grad():
            predicted_x_start = self.model_predictions(x, t, cond).pred_x_start
            predicted_x_start.detach_()

        # predict and take gradient step
        x_cond = predicted_x_start if self.self_condition else cond
        ...
        return loss.mean(), predicted_x_start, x
