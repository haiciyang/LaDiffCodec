...
def standardize_data(x):
        x = x / (torch.std(x) + 1e-10)
        return x

def is_clipping(x, clipping_threshold = 0.99):
    return torch.max(torch.abs(x)) > clipping_threshold
...
class Dataset_Libri(Dataset):
    ...
    # @ex.capture
    def __init__(self, task = 'train', seq_len_p_sec=5, data_folder_path='/data/hy17/librispeech/librispeech', data_proc='norm'): # 28539
